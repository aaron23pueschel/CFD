...
class Nozzle:
    ...
    def set_initial_conditions(self):
        self.mach = .1*np.ones((self.NI+1,self.NJ+1)) # only a left ghost node
    ...
    def set_ramp_BC(self):
        ...
        def generate_ramp_mesh(x_flat=1.0, x_ramp=2.0, height=1.0, angle_deg=5, 
    ...
    def set_normal_bcs(self):
        ...
        G[self.rho_idx,0,:]= 2*G[self.rho_idx,1,:] - G[self.rho_idx,2,:]

        T = self.V[self.p_idx,-2,:]/(self.V[self.rho_idx,-2,:]*self.R)
        rho0 = self.total_density(self.p0,self.R,T)
        
        G[self.rho_idx,-1,:]= 2*G[self.rho_idx,-2,:] - G[self.rho_idx,-3,:]
    ...
    def get_normal_directions(self,direction,UPWIND = True):
        if UPWIND:
            if direction=="right":
    ...
    def set_boundary_conditions(self):
        ...
        self.mach[:,0] = 4*np.ones_like(self.mach[:,0])
    ...
    def f_convective(self,i_plus_half=True,direction = "left"):
        #shift = self.FL_FR_FUNC(i_plus_half=i_plus_half)
    ...
    def FLUXL_FLUXR_FUNC(self,direction:str):
        ...
        def shift_LR(F): 
            ...
            p1 = self.psi_plus(F,-1+shift_indx,UP_DOWN=False)
            p2 = self.psi_minus(F,shift_indx,UP_DOWN=False)
            p3 = self.psi_minus(F,shift_indx+1,UP_DOWN=False)
            p4 = self.psi_plus(F,shift_indx,UP_DOWN=False)
        ...
        def shift_UD(F): 
            ...
            p1 = self.psi_plus(G,-1+shift_indx,UP_DOWN = True)
            
            p2 = self.psi_minus(G,shift_indx,UP_DOWN = True)
            p3 = self.psi_minus(G,shift_indx+1,UP_DOWN = True)
            p4 = self.psi_plus(G,shift_indx,UP_DOWN = True)
    ...
    def f_pressure_flux(self,i_plus_half=True,direction = "left",first_order=False):
    ...
    def compute_timestep(self):
        ...
        nx_psi_p1,ny_psi_p1 = self.get_normal_directions("left",UPWIND=True)
        nx_psi_p2,ny_psi_p2 = self.get_normal_directions("right",UPWIND=True)
        nx_psi = (nx_psi_p2+nx_psi_p1)/2
        ny_psi = (ny_psi_p2+ny_psi_p1)/2

        nx_eta_p1,ny_eta_p1 = self.get_normal_directions("up",UPWIND=True)
        nx_eta_p2,ny_eta_p2 = self.get_normal_directions("down",UPWIND=True)
    ...
    def iteration_step(self,return_error=True):
        ...
        if self.local_timestep:       
            self.U[:,1:-1,1:-1] = self.U[:,1:-1,1:-1]-(residual*self.delta_t/(self.AREA))
        else:
            self.U[:,1:-1,1:-1] = self.U[:,1:-1,1:-1]-(residual*self.delta_t/np.tile(Volume,(3,1)).T)
        # Update all 
        # Extrapolate left/right faces (x = 0 and x = -1), excluding boundaries in y
    ...
    def set_ramp_inflow(self,num_vals = 20):
        num_vals = num_vals +1
        #self.mach= np.abs(self.V[:,1])/np.sqrt(np.abs(self.gamma*self.V[:,2]/self.V[:,0]))
        #self.mach[0] = np.max([epsilon,2*self.mach[1]-self.mach[2]]) # Maybe dont need this line...
        #self.mach[0,:num_vals] = 4.1*np.ones_like(self.mach[0,:num_vals])
        self.mach[0,:num_vals] = 2*np.ones_like(self.mach[0,:num_vals])
        T = self.total_T(self.gamma,self.mach[0,:num_vals],self.T0)
        p_bndry = self.total_p(self.gamma,self.mach[0,:num_vals],self.p0)
        rho_bndry = self.total_density(p_bndry,self.R,T)

        nx,ny = self.get_normal_directions("down",UPWIND = True)
        u_bndry = nx[0,:num_vals]*self.total_velocity(self.gamma,self.mach[0,:num_vals],self.R,T)
        v_bndry = 0*self.total_velocity(self.gamma,self.mach[0,:num_vals],self.R,T)
        
        self.V[:,-1,:num_vals] = np.array([rho_bndry,u_bndry,v_bndry,p_bndry])
        self.V[:,:,-1] =self.V[:,:,-2] #np.array([self.extrapolate1(self.V)[1]])
        G[self.u_idx,-1,1:-1]
    ...
    def set_ramp_bcs(self,num_top=20):
        ...
        G[self.p_idx,0,(num_top+1):-1] = G[self.p_idx,1,(num_top+1):-1] 
        G[self.rho_idx,0,(num_top+1):-1] = G[self.rho_idx,1,(num_top+1):-1] 

        G[self.p_idx,1:-1,0] = G[self.p_idx,1:-1,1]
        G[self.rho_idx,1:-1,0] = G[self.rho_idx,1:-1,1]
