...
class Nozzle:
    ...
    def set_initial_conditions2(self,direction="down",boundary_mach=.01):
        ...
        rho = self.total_density(p,self.R,T)
        u = nx*self.total_velocity(self.gamma,self.mach,self.R,T)[1:-1,1:-1]
        v = ny*self.total_velocity(self.gamma,self.mach,self.R,T)[1:-1,1:-1]
    ...
    def f_convective(self,i_plus_half=True,direction = "left"):
        ...
        p_R = np.minimum(1e+4,p_R)
        p_L = np.minimum(1e+4,p_L)
        U_L = u_L*nx +ny*v_L
        U_R = u_R*nx +ny*v_R
    ...
    def fortran_function(self,direction = "left",upwind_scheme = "van leer"):
        Nx,Ny = self.get_normal_directions(direction=direction)
