...
class Nozzle:
    ...
    def set_initial_conditions(self):
        ...
        if self.flat_initial_mach:
            ...
            self.mach[1:-1] = 3*np.ones_like((9/10)*tempx+1)
    ...
    def set_boundary_conditions(self):
        ...
        self.mach = 4.1
        T = self.total_T(self.gamma,self.mach,self.T0)
        p_bndry = self.total_p(self.gamma,self.mach,self.p0)
        rho_bndry = self.total_density(p_bndry,self.R,T)
        u_bndry = self.total_velocity(self.gamma,self.mach,self.R,T)
    ...
    def iteration_step(self,return_error=True):
        #print("Inflow velocity:", self.V[0,1])
        ...
        Volume = np.ones_like((self.A[0:-1]+self.A[1:])*deltax/2)
        if self.damping_scheme==0:
