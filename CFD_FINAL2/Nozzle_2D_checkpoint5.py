...
class Nozzle:
    ...
    def set_initial_conditions(self):
        self.mach = 3*np.ones((self.NI+1,self.NJ+1)) # only a left ghost node
    ...
    def plot_primitive(self,type_="pressure"):
        ...
        if type_=="pressure":
            ...
            plt.colorbar()
        if type_=="velocity":
            plt.quiver(midpointyy,midpointxx,self.V[self.v_idx,1:-1,1:-1],self.V[self.u_idx,1:-1,1:-1])
        if type_=="density":
