...
class Nozzle:
    ...
    def set_initial_conditions(self):
        self.mach = 1.01*np.ones((self.NI+1,self.NJ+1)) # only a left ghost node
    ...
    def set_normal_bcs(self):
        ...
        G[self.u_idx,1:-1,0] = bottom[0]
        G[self.v_idx,1:-1,0] = bottom[1]
    ...
    def set_ramp_inflow(self,num_vals = 20):
        ...
        self.mach[0,:num_vals] = 1.1*np.ones_like(self.mach[0,:num_vals])
        ...
        nx,ny = self.get_normal_directions("up",UPWIND = False)
        u_bndry = self.total_velocity(self.gamma,self.mach[0,:num_vals],self.R,T)
    ...
    def set_boundary_conditions(self):
        ...
        self.mach[:,0] = 4.1*np.ones_like(self.mach[:,0])
    ...
    def FLUXL_FLUXR_FUNC(self,direction:str):
        ...
        def shift_UD(F): 
            ...
            p1 = self.psi_plus(F.T,-1+shift_indx)
            p2 = self.psi_minus(F.T,shift_indx)
            p3 = self.psi_minus(F.T,shift_indx+1)
            p4 = self.psi_plus(F.T,shift_indx)
    ...
    def compute_timestep(self):
        ...
        nx_psi_p1,ny_psi_p1 = self.get_normal_directions("left")
        nx_psi_p2,ny_psi_p2 = self.get_normal_directions("right")
        nx_psi = (nx_psi_p2+nx_psi_p1)/2
        ny_psi = (ny_psi_p2+ny_psi_p1)/2

        nx_eta_p1,ny_eta_p1 = self.get_normal_directions("up")
        nx_eta_p2,ny_eta_p2 = self.get_normal_directions("down")
    ...
    def iteration_step(self,return_error=True):
        ...
        # Back face (index -1) extrapolated from last two interior slices
        self.U[:, 1:-1, -1] = 2 * self.U[:, 1:-1, -2] - self.U[:, 1:-1, -3]
        
        #_,self.U[-1] = self.extrapolate1(self.U)
        #self.set_boundary_conditions()
